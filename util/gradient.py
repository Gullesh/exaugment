...
def igradbox(net,a,l, length=16, p=0.5,rescle = True, arng=[7*8,7*18], sal=False):
    '''
    a : batch of images
    p: probability of gradient happening
    '''
    ...
    n = saliency.view(bsize, -1)
    _, ll = torch.max(n,1)
    x, y = torch.div(ll, 224, rounding_mode='floor'), torch.remainder(ll,224)
    y1 = torch.clamp(y - (torch.div(lengt, 2, rounding_mode='floor')), 0, 224)
    y2 = torch.clamp(y + (torch.div(lengt, 2, rounding_mode='floor')), 0, 224)
    x1 = torch.clamp(x - (torch.div(lengt, 2, rounding_mode='floor')), 0, 224)
    x2 = torch.clamp(x + (torch.div(lengt, 2, rounding_mode='floor')), 0, 224)
    new = torch.empty((1,224,224)).cuda()
    if sal:
        for ii in range(bsize):
        ...
        for ii in range(bsize):
            mask = torch.ones((1, 224,224)).cuda()
            mask[0, x1[ii]: x2[ii], y1[ii]: y2[ii]] = 0.
            new = torch.cat((new,mask),dim = 0)
    ...
    fo = a*AAA
    if rescle:
        fo = fo * (1/((lengt**2/224**2).reshape(bsize,1,1,1)))
    rnd = ((torch.rand(size=(bsize,1,1,1)) < p).int()).cuda()
    foo = rnd*fo
    aa = ((1-rnd)*a) + foo
    net.train()
