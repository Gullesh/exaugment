...
if args.data =='cifar10':
    ...
    print('what the fuck?')
history = {'train_acc':[],'test_acc':[], 'best_acc':[]}
for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(len(dataset)))):
    print('Fold {}'.format(fold + 1))
    best_acc = 0  # best test accuracy
    last_acc = 0
    trainacc = 0
    traset = torch.utils.data.Subset(dataset, indices=train_idx)
    tesset = torch.utils.data.Subset(dataset, indices=val_idx)
    trset = MyDataset(traset, transform=train_transform)
    tsset = MyDataset(tesset, transform=transform_test)
    testloader = torch.utils.data.DataLoader(tsset, batch_size=args.batch_size, shuffle=False, num_workers=args.nworkers)
    trainloader = torch.utils.data.DataLoader(trset, batch_size=args.batch_size, shuffle=True, num_workers=args.nworkers)
    ...
    history['test_acc'].append(last_acc)
